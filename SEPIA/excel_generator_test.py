...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 14:18:06 2024
...
def build_filename(simpl,cluster,opt,sector_opt,planning_horizon):
    ...
    return prefix+"s{simpl}_{cluster}_{opt}_{sector_opt}_{planning_horizon}.nc".format(
        simpl=simpl,
        cluster=cluster,
        opt=opt,
...
def load_files(study, planning_horizons, simpl, cluster, opt, sector_opt):
    files = {}
...
loaded_files = load_files(study, planning_horizons, simpl, cluster, opt, sector_opt)
year = 2019
def process_network(simpl, cluster, opt, sector_opt, planning_horizon, country):
        results_dict = {}
        ...
        if planning_horizon == 2020 or study == 'sensitivity_analysis_seq_0':
            H2_nonenergyy = 0
        if study == 'ncdr':
            clever_industry = (
        ...
        # else:
        #  dac_value = n.links_t.p0.filter(like="DAC").sum().sum()/1e6
...
def prepare_emissions(simpl, cluster, opt, sector_opt, planning_horizon, country):
        '''
        ...
        n = loaded_files[planning_horizon]
        fn = "/home/umair/pypsa-eur_repository/data/costs_2050.csv"
...
def write_to_excel(simpl, cluster, opt, sector_opt, planning_horizons,countries):
    '''
    ...
    for country in countries:
     ...
     # if country != 'EU':
        merged_df = process_network(simpl, cluster, opt, sector_opt, planning_horizons[0], country)
        merged_df = merged_df[country]

        for planning_horizon in planning_horizons[1:]:
            temp = process_network(simpl, cluster, opt, sector_opt, planning_horizon, country)
            temp = temp[country]
        ...
        filename=f"/home/umair/pypsa-eur_repository/results/{study}/sepia/inputs{country}.xlsx"
    ...
    for country in countries:
     merged_emissions = prepare_emissions(simpl, cluster, opt, sector_opt, planning_horizons[0], country)
     merged_emissions = merged_emissions[country]
     for planning_horizon in planning_horizons[1:]:
            temp = prepare_emissions(simpl, cluster, opt, sector_opt, planning_horizon, country)
            temp = temp[country]
...
for planning_horizon in planning_horizons:
 for country in countries:
    prepare_emissions(simpl, cluster, opt, sector_opt, planning_horizon, country)
    process_network(simpl, cluster, opt, sector_opt, planning_horizon, country)
    
# process_countries(countries,study)

write_to_excel(simpl, cluster, opt, sector_opt, planning_horizons,countries)
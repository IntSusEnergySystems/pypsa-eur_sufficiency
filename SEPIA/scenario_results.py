...
def scenario_capacities(country):
    ...
    combined_df = pd.merge(caps_suff, caps_ref, on='tech', how='outer', suffixes=('_suff', '_ref'))
...
def create_combined_scenario_chart_country(country, output_folder='results/scenario_results/'):
    ...
    if scenario_plots["Annual Operational Costs"] == True:
     main_content += f"<div id='{display_country} - Annual Operational Costs'><h2>{display_country} - Annual Operational Costs</h2>{bar_chart_operational.to_html(full_html=False, include_plotlyjs='cdn')}</div>"
    if scenario_plots["Cummulative Investment Costs"] == True:
     main_content += f"<div id='{display_country} - Cumulative Investment Costs (2020-2050)'><h2>{display_country} - Cumulative Investment Costs (2020-2050)</h2>{bar_chart_cumulative.to_html(full_html=False, include_plotlyjs='cdn')}</div>"
    if scenario_plots["Capacities"] == True:

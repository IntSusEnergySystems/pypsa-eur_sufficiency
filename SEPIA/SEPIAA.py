...
def prepare_sepia(countries):
 ...
 for country in ALL_COUNTRIES:
    ...
    # bm_liq = flows['enc_pe','pet_fe',''].squeeze().rename_axis(None)
    # bm_liq_cc = flows['enc_pe','pet_fe','cc'].squeeze().rename_axis(None)
    # fischer_trop = flows['hyd_se','pet_fe',''].squeeze().rename_axis(None)
    # elctro = flows['hyd_se','pet_fe','bio'].squeeze().rename_axis(None)
    # elctro_bm = flows['enc_pe','pet_fe','bio'].squeeze().rename_axis(None)
    # if country == 'EU':
    for en_code in ['pet']: 
    ...
    # else:
    #   for en_code in ['pet']: 
    #    flows[(en_code+'_pe',en_code+'_fe','')] = fec[en_code+'_fe']-bm_liq-fischer_trop-elctro_bm-bm_liq_cc
    ...
    if country != 'EU':
     ...
     bm_potentials_eu = bm_potentialss.loc[:, country].values.sum()
     for en_code in ['enc']:
      ...
      for en_code in ['enc']:
       flows[('prod',en_code+'_pe','')] = bm_potentials_eu
       flows.loc['2020', ('prod', en_code + '_pe', '')] = value_bm_data['2020'].sum()
       imp_values = fec_p[en_code + '_pe'] - bm_potentials_eu
       imp_values2020 = fec_p[en_code + '_pe']['2020'] - (value_bm_data['2020'].sum())
       flows[('imp', en_code + '_pe', '')] = imp_values
       flows.loc['2020', ('imp', en_code + '_pe', '')] = imp_values2020
    ...
    # for en_code in ['oil']: 
    #   value_bml = flows_co2[('bm_ghg', en_code + '_ghg', '')].squeeze().rename_axis(None)
    #   value_fisc = flows_co2[('stm', en_code + '_ghg', '')].squeeze().rename_axis(None)
    #   value_efuel = flows_co2[('bm_ghg', en_code + '_ghg', 'bio')].squeeze().rename_axis(None)
    #   value_pet_emm = tot_em[en_code+'_ghg']
    #   flows_co2[('fol_ghg', en_code + '_ghg', '')] = value_pet_emm-value_bml-value_fisc-value_efuel
    if country == 'EU':
      for en_code in ['gas']:
 ...
 def generate_results(flows, tot_results, country):
    ...
    ghg_sector['wcc_ghg'] = -ghg_sector['wcc_ghg']
    ...
    ghg_source['wcc_ghg'] = -ghg_source['wcc_ghg']

...
def create_sankey(flows, nodes, processes, main_params, interval_year=5, title="Sankey diagram"):
    ...
    for i, year in enumerate(years):
        ...
        pad_value = 40 if year == 2020 else 15
        link_colors = np.where(
            year_links['Target'].isin(['per', 'use']),
            year_nodes.loc[year_links['Target'], 'ColorOpacity'].values,
            year_nodes.loc[year_links['Source'], 'ColorOpacity'].values)
        sk = go.Sankey(
            valueformat = ".0f",
            valuesuffix = " TWh",
            visible = False,
    ...
    add_sankey_label(fig, 0, 'Primary energy')
    # add_sankey_label(fig, nodes.loc['cms_pe','PositionX'], 'Primary energy')
    add_sankey_label(fig, 0.25, 'Secondary energy & networks')
    add_sankey_label(fig, 0.55, 'Final energy')
    add_sankey_label(fig, 0.8, 'Demand sector')
    add_sankey_label(fig, 1, 'Useful energy')
